...
const studioPrompterList = document.getElementById('content-prompter')!;
...
const toggleMultitrack = document.getElementById('toggle-multitrack')!;
const contentMultitrack = document.getElementById('content-multitrack')!;
const caretMultitrack = document.getElementById('caret-multitrack')!;

const toggleAudioeditor = document.getElementById('toggle-audioeditor')!;
const contentAudioeditor = document.getElementById('content-audioeditor')!;
const caretAudioeditor = document.getElementById('caret-audioeditor')!;
const studioAudioEditor = document.getElementById('studio-audio-editor')!;

const togglePrompter = document.getElementById('toggle-prompter')!;
const contentPrompter = document.getElementById('content-prompter')!;
const caretPrompter = document.getElementById('caret-prompter')!;

function setupAccordion(toggle: HTMLElement, content: HTMLElement, caret: HTMLElement, defaultOpen = true) {
  toggle.addEventListener('click', () => {
    const isHidden = content.style.display === 'none';
    if (isHidden) {
      content.style.display = content.id === 'content-multitrack' ? 'block' : 'flex';
...
setupAccordion(toggleMultitrack, contentMultitrack, caretMultitrack, true);
setupAccordion(toggleAudioeditor, contentAudioeditor, caretAudioeditor, false);
setupAccordion(togglePrompter, contentPrompter, caretPrompter, true);
...
async function generateWaveform(url: string) {
  ...
  try {
    ...
    nleWaveformCanvas.width = numBars;
    nleWaveformCanvas.height = 100;
    ...
    if (studioCtx) {
      studioCanvas.width = numBars;
      studioCanvas.height = 100;
      studioCtx.fillStyle = 'rgba(255,255,255,0.4)';
    }
    const studioBarWidth = 1;
    
    const ratio = editorCurrentDuration && editorCurrentDuration > 0 ? (audioBuffer.duration / editorCurrentDuration) : 1;
    nleWaveformCanvas.style.width = `${ratio * 100}%`;
    ...
    for (let i = 0; i < numBars; i++) {
      ...
      if (studioCtx) {
        studioCanvas.style.width = `${ratio * 100}%`;
...
function startRoomChannel(code: string) {
  if (roomChannel) {
    roomChannel.unsubscribe();
  }
  roomChannel = supabase.channel(`room-${code}`);
  roomChannel.on('broadcast', { event: 'app_message' }, (payload) => {
    handleIncomingMessage(payload.payload.type, payload.payload.payload);
  });
...
// --- Sync Logic ---
function handleIncomingMessage(type: string, payload: any) {
...
function renderStudioPrompter() {
  ...
  sorted.forEach(dialogue => {
    ...
    let html = `
      ...
      <div style="font-size:1.1rem; line-height:1.4; word-break: break-word; overflow-wrap: anywhere;">${dialogue.text}</div>
    `;
...
// --- Web Audio API para Mixer del Estudio ---
let studioAudioCtx: AudioContext | null = null;
const mixerStudioPlayers: Record<string, any> = {};

function initStudioAudioContext() {
  if (!studioAudioCtx) {
    studioAudioCtx = new (window.AudioContext || (window as any).webkitAudioContext)();
  }
...
let aeActiveClipId: string | null = null;
const aeWaveform = document.getElementById('ae-waveform') as HTMLCanvasElement;
const aeVol = document.getElementById('ae-vol') as HTMLInputElement;
const aePitch = document.getElementById('ae-pitch') as HTMLInputElement;
const aeEqLow = document.getElementById('ae-eq-low') as HTMLInputElement;
const aeEqMid = document.getElementById('ae-eq-mid') as HTMLInputElement;
const aeEqHigh = document.getElementById('ae-eq-high') as HTMLInputElement;
const aeFadeIn = document.getElementById('ae-fade-in') as HTMLButtonElement;
const aeFadeOut = document.getElementById('ae-fade-out') as HTMLButtonElement;
const aePlay = document.getElementById('ae-play') as HTMLButtonElement;
const aeDelete = document.getElementById('ae-delete') as HTMLButtonElement;
const aeVolVal = document.getElementById('ae-vol-val')!;
const aePitchVal = document.getElementById('ae-pitch-val')!;

function updateAeUI() {
  if (!aeActiveClipId) return;
  const clip = studioRecordedClips.find(c => c.id === aeActiveClipId);
  if (!clip) return;
  
  aeVol.value = (clip.volume ?? 1).toString();
  aeVolVal.textContent = Math.round((clip.volume ?? 1) * 100) + '%';
  
  aePitch.value = (clip.pitch ?? 1).toString();
  aePitchVal.textContent = (clip.pitch ?? 1) + 'x';
  
  aeEqLow.value = (clip.eqLow ?? 0).toString();
  aeEqMid.value = (clip.eqMid ?? 0).toString();
  aeEqHigh.value = (clip.eqHigh ?? 0).toString();
  
  aeFadeIn.classList.toggle('primary', !!clip.fadeIn);
  aeFadeIn.classList.toggle('secondary', !clip.fadeIn);
  aeFadeOut.classList.toggle('primary', !!clip.fadeOut);
  aeFadeOut.classList.toggle('secondary', !clip.fadeOut);
  
  // Draw basic waveform on canvas
  const ctx = aeWaveform.getContext('2d');
  if (ctx) {
    ctx.clearRect(0, 0, aeWaveform.width, aeWaveform.height);
    if (clip.blobUrl) {
      drawMiniWaveform(clip.blobUrl, aeWaveform, '#ffffff');
    }
  }
}

[aeVol, aePitch, aeEqLow, aeEqMid, aeEqHigh].forEach(input => {
  input.addEventListener('input', () => {
    if (!aeActiveClipId) return;
    const clip = studioRecordedClips.find(c => c.id === aeActiveClipId);
    if (!clip) return;
    
    if (input === aeVol) { clip.volume = parseFloat(aeVol.value); aeVolVal.textContent = Math.round(clip.volume * 100) + '%'; }
    if (input === aePitch) { clip.pitch = parseFloat(aePitch.value); aePitchVal.textContent = clip.pitch + 'x'; }
    if (input === aeEqLow) clip.eqLow = parseFloat(aeEqLow.value);
    if (input === aeEqMid) clip.eqMid = parseFloat(aeEqMid.value);
    if (input === aeEqHigh) clip.eqHigh = parseFloat(aeEqHigh.value);
    
    // Apply immediately to playing node if exists
    if (mixerStudioPlayers[clip.id]) {
       const p = mixerStudioPlayers[clip.id];
       p.audio.playbackRate = clip.pitch ?? 1;
       p.gain.gain.value = clip.volume ?? 1;
       p.low.gain.value = clip.eqLow ?? 0;
       p.mid.gain.value = clip.eqMid ?? 0;
       p.high.gain.value = clip.eqHigh ?? 0;
    }
  });
});

aeFadeIn.addEventListener('click', () => {
  const clip = studioRecordedClips.find(c => c.id === aeActiveClipId);
  if (clip) { clip.fadeIn = !clip.fadeIn; updateAeUI(); }
});
aeFadeOut.addEventListener('click', () => {
  const clip = studioRecordedClips.find(c => c.id === aeActiveClipId);
  if (clip) { clip.fadeOut = !clip.fadeOut; updateAeUI(); }
});

aePlay.addEventListener('click', () => {
  const clip = studioRecordedClips.find(c => c.id === aeActiveClipId);
  if (clip && clip.blobUrl) {
    const audio = new Audio(clip.blobUrl);
    audio.playbackRate = clip.pitch ?? 1;
    audio.volume = Math.min(1, clip.volume ?? 1);
    audio.play().catch(e => console.error('Error playing clip:', e));
  }
});

aeDelete.addEventListener('click', () => {
  if (confirm('¿Seguro que quieres borrar este clip?')) {
    studioRecordedClips = studioRecordedClips.filter(c => c.id !== aeActiveClipId);
    if (aeActiveClipId && mixerStudioPlayers[aeActiveClipId]) {
      mixerStudioPlayers[aeActiveClipId].audio.pause();
      delete mixerStudioPlayers[aeActiveClipId];
    }
    aeActiveClipId = null;
    document.getElementById('content-audioeditor')!.style.display = 'none';
    document.getElementById('caret-audioeditor')!.style.transform = 'rotate(-90deg)';
    renderStudioMultitrack();
  }
});
...
function startStudioLoop() {
  ...
  const loop = () => {
    ...
    // Mixer Logic
    if (isMixerMode) {
      initStudioAudioContext();
      studioRecordedClips.forEach(clip => {
        let player = mixerStudioPlayers[clip.id];
        if (!player) {
          const audio = new Audio(clip.blobUrl);
          ...
          const low = studioAudioCtx!.createBiquadFilter();
          low.type = 'lowshelf';
          low.frequency.value = 320;
          
          const mid = studioAudioCtx!.createBiquadFilter();
          mid.type = 'peaking';
          mid.frequency.value = 1000;
          mid.Q.value = 0.5;
          
          const high = studioAudioCtx!.createBiquadFilter();
          high.type = 'highshelf';
          ...
          player = { audio, source, gain, low, mid, high };
          mixerStudioPlayers[clip.id] = player;
        }
        // Update real-time node properties
        player.audio.playbackRate = clip.pitch ?? 1;
        player.low.gain.value = clip.eqLow ?? 0;
        player.mid.gain.value = clip.eqMid ?? 0;
        player.high.gain.value = clip.eqHigh ?? 0;
        
        let targetGain = clip.volume ?? 1;
        const targetTime = time - clip.startTime;
        
        // Fades
        if (clip.fadeIn && targetTime < 2) {
          targetGain = targetGain * (targetTime / 2);
        }
        if (clip.fadeOut && (clip.endTime - time) < 2) {
          targetGain = targetGain * ((clip.endTime - time) / 2);
        }
        player.gain.gain.value = targetGain;
        
        if (time >= clip.startTime && time <= clip.endTime) {
          if (player.audio.paused) {
            if (Math.abs(player.audio.currentTime - targetTime) > 0.2) player.audio.currentTime = targetTime;
            player.audio.play().catch((e: any) => console.error(e));
          }
        } else {
...
document.addEventListener('keydown', (e) => {
  if (e.code === 'Space' && !viewStudio.classList.contains('hidden') && studioMode === 'freestyle') {
    if (document.activeElement?.tagName === 'INPUT' || document.activeElement?.tagName === 'TEXTAREA') return;
    if (e.repeat) return;
    e.preventDefault();
    if (!isGlobalRecording) {
      btnGlobalRecord.dispatchEvent(new MouseEvent('mousedown', { button: 0 }));
    }
  }
});
...
function renderStudioMultitrack() {
  ...
  if (studioRuler) {
    ...
    for (let i = 0; i <= numMarkers; i++) {
      ...
      marker.style.borderLeft = '1px solid rgba(255,255,255,0.3)';
      marker.style.bottom = '0';
      marker.style.fontSize = '0.6rem';
      marker.style.color = 'var(--text-muted)';
      marker.style.paddingLeft = '2px';
      marker.style.pointerEvents = 'none';
      marker.textContent = formatTime(time).substring(3, 8);
  ...
  studioRecordedClips.forEach(clip => {
    ...
    if (clip.id === aeActiveClipId) {
      div.style.border = '2px solid white';
      div.style.boxShadow = '0 0 10px rgba(255,255,255,0.5)';
    }
    
    div.addEventListener('mousedown', (e) => {
      aeActiveClipId = clip.id;
      aeDragState.isDragging = true;
      aeDragState.startX = e.clientX;
      aeDragState.startY = e.clientY;
      aeDragState.initialStartTime = clip.startTime;
      aeDragState.initialTrack = clip.trackIndex;
    ...
    div.addEventListener('click', (e) => {
       aeActiveClipId = clip.id;
       updateAeUI();
       document.getElementById('content-audioeditor')!.style.display = 'flex';
       document.getElementById('caret-audioeditor')!.style.transform = 'rotate(0deg)';
       renderStudioMultitrack(); // Re-render to show border
       e.stopPropagation();
    });
...
// Global Drag State for Studio Timeline
const aeDragState = {
  isDragging: false,
  startX: 0,
  startY: 0,
...
document.addEventListener('mousemove', (e) => {
  if (!aeDragState.isDragging || !aeActiveClipId) return;
  const clip = studioRecordedClips.find(c => c.id === aeActiveClipId);
  if (!clip) return;
  
  const dur = editorVidCtrl.duration || 1;
  const dx = e.clientX - aeDragState.startX;
  const dy = e.clientY - aeDragState.startY;
  const timelineWidth = document.getElementById('studio-multitrack-timeline')!.offsetWidth;
  
  const timeOffset = (dx / timelineWidth) * dur;
  let newStart = aeDragState.initialStartTime + timeOffset;
  ...
  // Track changing
  const trackHeight = document.querySelector('.studio-track[data-track="0"]')!.getBoundingClientRect().height;
  const trackOffset = Math.round(dy / trackHeight);
  let newTrack = aeDragState.initialTrack + trackOffset;
...
document.addEventListener('mouseup', () => {
  if (aeDragState.isDragging) {
    aeDragState.isDragging = false;
  }
});
